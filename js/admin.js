...
async function loadBappEligibilityDataT2(force = false) {
  ...
  try {
    ...
    const realisasiPmlMapG4 = {};
    achievements.forEach(a => {
      realisasiMapG1[a.kode_sls_gabungan] = parseInt(a.capaian2_g1 || a.capaian2 || 0) || 0;
      realisasiMapG2[a.kode_sls_gabungan] = parseInt(a.capaian2_g2 || 0) || 0;
      realisasiMapG3[a.kode_sls_gabungan] = parseInt(a.capaian2_g3 || 0) || 0;
      realisasiMapG4[a.kode_sls_gabungan] = parseInt(a.capaian2_g4 || 0) || 0;
      realisasiPmlMapG1[a.kode_sls_gabungan] = parseInt(a.capaian2_pml_g1 || a.capaian2_pml || 0) || 0;
      realisasiPmlMapG2[a.kode_sls_gabungan] = parseInt(a.capaian2_pml_g2 || 0) || 0;
      realisasiPmlMapG3[a.kode_sls_gabungan] = parseInt(a.capaian2_pml_g3 || 0) || 0;
      realisasiPmlMapG4[a.kode_sls_gabungan] = parseInt(a.capaian2_pml_g4 || 0) || 0;
    });
    ...
    const getPplStatusT2 = (pplId, gel) => {
      ...
      const eligible = (pplTargetSum > 0 && pplRealisasiSum >= pplTargetSum) || (slsCodes.length > 0 && pplCoverage === slsCodes.length);
      if (isOnHold(pplId, gel)) {
        return { eligible: false, onHold: true };
      }
      return { eligible, onHold: false };
    };
      ...
      userProfiles.forEach(u => {
        if (u.role === 'pml') {
          ...
          const getPmlGelEligibility = (g) => {
            ...
            const pct = pmlTargetSum > 0 ? (pmlRealisasiSum / pmlTargetSum) : 0;
            const eligible = (pct >= 1.0) || (totalPmlSls > 0 && visitedPml === totalPmlSls);
            return eligible && !isOnHold(u.id, g);
          };
...
function openBappModal() {
  currentBappTermin = 1;
  const modal = document.getElementById('bappModal');
  const title = document.getElementById('bappModalTitle');
  if (title) title.textContent = 'Tabel BAPP Pencairan Termin I';
  if (modal) {
    modal.classList.add('open');
...
async function loadBAPPData() {
  ...
  try {
    ...
    const selectCols = currentBappTermin === 2
      ? 'id, profile_id, kode_kec, crop_top, crop_bottom, created_at, profiles:profile_id(nama, role, sobatid), wilayah_kec:kode_kec(nmkec)'
      : 'id, profile_id, kode_kec, crop_top, crop_bottom, created_at, profiles:profile_id(nama, role, sobatid), wilayah_kec:kode_kec(nmkec)';
...
function updateSelectedBappCount() {
  ...
  function printBAPP(rows) {
    ...
    loadJsPDF(async () => {
      ...
      if (currentBappTermin === 2) {
        ...
        try {
          const sobatIds = sortedRows.map(r => String(r.profiles?.sobatid).trim()).filter(Boolean);
          if (sobatIds.length > 0) {
            const { data } = await db.from('no_surat_se').select('sobatid, no_spk').in('sobatid', sobatIds);
            (data || []).forEach(n => { noSuratMap[String(n.sobatid).trim()] = n; });
          }
          const userIds = sortedRows.map(r => r.id);
          const { data: slsData } = await db.from('user_sls_termin2').select('user_id, kode_sls').in('user_id', userIds).eq('status', 'aktif');
          userSlsData = slsData || [];
        } catch (err) {
      ...
      `;
        ...
        const role = r.profiles?.role || 'ppl';
        const isPml = role === 'pml';
        const nama = r.profiles?.nama || '.........................................';
        const nik = r.profiles?.nik || '................';
        const sobatid = String(r.profiles?.sobatid).trim();
        const kecamatan = r.wilayah_kec?.nmkec || '.........................................';

        if (i > 0) {
          pdf.addPage('a4', orientationPdf);
        }

        if (currentBappTermin === 2) {
          const noEntry = noSuratMap[sobatid] || {};
          const spkNo = noEntry.no_spk || '.........................';
          const nomorSurat = `Nomor: ${spkNo}`;

          const pSlS = userSlsData.filter(s => s.user_id === r.id);
          const jmlSls = pSlS.length;

          pdf.setFont('Bookman', 'bold');
          pdf.setFontSize(11);
          pdf.text("BERITA ACARA PEMERIKSAAN HASIL PEKERJAAN TERMIN II", 105, 30, { align: 'center' });
          pdf.text("PETUGAS LAPANGAN", 105, 35, { align: 'center' });
          pdf.text("SENSUS EKONOMI 2026", 105, 40, { align: 'center' });
          pdf.setFont('Bookman', 'normal');
          pdf.text(nomorSurat, 105, 50, { align: 'center' });

          let y = 60;
          pdf.setFontSize(10);
          const introText = "Pada hari ini ...., tanggal ...., bulan ........, tahun dua ribu dua puluh enam (...-...-2026), berdasarkan Perjanjian Kerja pada Badan Pusat Statistik Kabupaten Lebak Nomor: ... , bertempat di Lebak, kami yang bertanda tangan di bawah ini:";
          const splitIntro = pdf.splitTextToSize(introText, 160);
          pdf.text(splitIntro, 25, y);
          y += (splitIntro.length * 5) + 5;

          pdf.text("1.", 25, y);
          pdf.text("Nama", 30, y); pdf.text(": YULIAN SARWO EDI", 50, y); y += 5;
          pdf.text("NIP", 30, y); pdf.text(": 197707101999121001", 50, y); y += 5;
          pdf.text("Jabatan", 30, y); pdf.text(": Ketua Tim Pelaksana Sensus Ekonomi 2026 BPS Kabupaten Lebak", 50, y); y += 5;
          pdf.text("dalam hal ini mewakili:", 30, y); y += 5;
          pdf.text("Nama", 30, y); pdf.text(": NING SRI LESTARI", 50, y); y += 5;
          pdf.text("NIP", 30, y); pdf.text(": 198107062002122002", 50, y); y += 5;
          const wrapPPK = pdf.splitTextToSize(": PPK BPS Kabupaten Lebak, selanjutnya disebut sebagai PIHAK PERTAMA.", 140);
          pdf.text("Jabatan", 30, y); pdf.text(wrapPPK, 50, y);
          y += (wrapPPK.length * 5) + 2;

          pdf.text("2.", 25, y);
          pdf.text("Nama", 30, y); pdf.text(`: ${nama.toUpperCase()}`, 50, y); y += 5;
          pdf.text("NIK", 30, y); pdf.text(`: ${nik}`, 50, y); y += 5;
          const jabatanStr = isPml ? "PETUGAS PEMERIKSA LAPANGAN Sensus Ekonomi 2026" : "PETUGAS LAPANGAN Sensus Ekonomi 2026";
          pdf.text("Jabatan", 30, y); pdf.text(`: ${jabatanStr}`, 50, y); y += 5;
          const wrapPihak2 = pdf.splitTextToSize("bertindak untuk dan atas namanya sendiri, selanjutnya disebut sebagai PIHAK KEDUA.", 160);
          pdf.text(wrapPihak2, 30, y);
          y += (wrapPihak2.length * 5) + 5;

          pdf.text("Dengan ini menyatakan:", 25, y); y += 5;
          const klausa = [
            `PIHAK KEDUA telah melaksanakan pekerjaan Pendataan Lapangan Sensus Ekonomi 2026 pada Badan Pusat Statistik Kabupaten Lebak berdasarkan Perjanjian Kerja Nomor: ..., sesuai dengan target pekerjaan termin II yang ditetapkan sebesar ${jmlSls} SLS/Sub-SLS.`,
            ...
            `Pekerjaan Hasil pekerjaan Pendataan Lapangan Sensus Ekonomi 2026 sebagaimana dimaksud dalam angka 3, berupa dokumen hasil Pendataan Lapangan Sensus Ekonomi 2026, sejumlah ${jmlSls} SLS/Sub-SLS.`,
            `Pekerjaan pendataan sebagaimana dimaksud pada angka 4 yang memerlukan pemeriksaan lanjutan, akan dilakukan pengecekan, perubahan, dan/atau kunjungan kembali ke lapangan merujuk pada perjanjian yang ditandatangani oleh PARA PIHAK.`
          ];
          klausa.forEach((k, idx) => {
            pdf.text(`${idx + 1}.`, 25, y);
            const w = pdf.splitTextToSize(k, 150);
            pdf.text(w, 30, y);
            y += (w.length * 5) + 3;
          });
          ...
          const sigY1 = y;
          pdf.text("PIHAK PERTAMA,", 55, sigY1, { align: 'center' });
          pdf.text("PIHAK KEDUA,", 155, sigY1, { align: 'center' });
          pdf.text("Pejabat Pembuat Komitmen", 55, sigY1 + 5, { align: 'center' });
          if (ttdNingBase64) pdf.addImage(ttdNingBase64, 'PNG', 35, sigY1 + 10, 40, 20);
          pdf.setFont('Bookman', 'bold');
          pdf.text("NING SRI LESTARI", 55, sigY1 + 35, { align: 'center' });
          pdf.setFont('Bookman', 'normal');
          pdf.text("NIP. 198107062002122002", 55, sigY1 + 40, { align: 'center' });
          pdf.setFont('Bookman', 'bold');
          pdf.text(nama.toUpperCase(), 155, sigY1 + 35, { align: 'center' });
        ...
        if (currentBappTermin === 2 && r.screenshot_uninstall) {
          // Render both Capaian and Uninstall screenshots side-by-side
          const [res1, res2] = await Promise.all([
            prepareCroppedImage(r.screenshot, r.crop_top, r.crop_bottom),
            prepareCroppedImage(r.screenshot_uninstall, r.crop_top_uninstall || 0, r.crop_bottom_uninstall || 100)
          ]);
          ...
          // Slot 1 (Capaian)
          if (res1) {
            const w1 = Math.min(maxBoxW, boxH / res1.ratio);
            const x1 = 20 + (maxBoxW - w1) / 2;
            pdf.addImage(res1.base64, 'PNG', x1, 28.0, w1, boxH);
            pdf.rect(x1, 28.0, w1, boxH, 'D');
          } else {
            pdf.rect(20, 28.0, maxBoxW, boxH, 'D');
            pdf.setFont('times', 'italic');
            pdf.setFontSize(9);
            pdf.text('[Bukti Capaian FASIH Tidak Tersedia]', 80, 28.0 + 32, { align: 'center' });
          }

          // Slot 2 (Uninstall)
          if (res2) {
            const w2 = Math.min(maxBoxW, boxH / res2.ratio);
            const x2 = 157 + (maxBoxW - w2) / 2;
            pdf.addImage(res2.base64, 'PNG', x2, 28.0, w2, boxH);
            pdf.rect(x2, 28.0, w2, boxH, 'D');
          } else {
            pdf.rect(157, 28.0, maxBoxW, boxH, 'D');
            pdf.setFont('times', 'italic');
            pdf.setFontSize(9);
            pdf.text('[Bukti Uninstall FASIH Tidak Tersedia]', 217, 28.0 + 32, { align: 'center' });
          }
  ...
  /**
   * 2. Hitung Kelayakan Termin 2 secara Murni dari capaian_termin2 & user_sls_termin2
   */
  async function loadBappEligibilityDataT2(force = false) {
    if (isBappEligibilityLoadedT2 && !force) return;
    try {
      // Pastikan allUsers terisi penuh
      if (!allUsers || allUsers.length === 0) {
        await loadUsers();
      }
      ...
      userSlsT2.forEach(us => {
        if (!userSlsMap[us.user_id]) userSlsMap[us.user_id] = [];
        userSlsMap[us.user_id].push(us.kode_sls);
        if (!pplKecMap[us.user_id] && us.kode_sls) {
          pplKecMap[us.user_id] = us.kode_sls.substring(4, 7);
        }
      });

      // 3. Ambil data capaian_termin2 (Target & Hasil Verif)
      let capaianT2 = [];
      let fromCap = 0;
      let hasMoreCap = true;
      while (hasMoreCap) {
      ...
      const targetMap = {};
      const hasilMapG = { 1: {}, 2: {}, 3: {}, 4: {} };
      const slsSelesaiMapG = { 1: {}, 2: {}, 3: {}, 4: {} };

      capaianT2.forEach(c => {
        targetMap[c.kode_subsls] = c.targetsls || 0;
        [1, 2, 3, 4].forEach(g => {
          hasilMapG[g][c.kode_subsls] = c[`hasilverif${g}`] || 0;
          slsSelesaiMapG[g][c.kode_subsls] = c[`sls_selesai${g}`] || 0;
        });
      });

      // 4. Ambil hold honorarium khusus Termin 2
      let holdsRaw = [];
      try {
        const { data, error } = await db.from('honorarium_hold')
          ...
          .eq('is_active', true);
        if (!error && data) {
          holdsRaw = data.filter(h => h.termin === 2);
        }
      } catch (e) {
        console.warn('Gagal memuat hold Termin 2:', e);
      }

      const holdSet = new Set();
      holdsRaw.forEach(h => {
        if (h.gelombang === null || h.gelombang === undefined) {
          [1, 2, 3, 4].forEach(g => holdSet.add(`${h.user_id}:${g}`));
        } else {
          holdSet.add(`${h.user_id}:${h.gelombang}`);
        }
      });

      const isOnHold = (uid, g) => holdSet.has(`${uid}:${g}`);
      ...
      // Evaluasi kelayakan PPL & PML per gelombang
      let userProfiles = (allUsers && allUsers.length > 0) ? allUsers : [];
      if (userProfiles.length === 0) {
        let fromP = 0;
        let hasMoreP = true;
        while (hasMoreP) {
          const { data, error } = await db.from('profiles').select('id, sobatid, nama, email_ref, role').in('role', ['ppl', 'pml']).eq('is_active', true).range(fromP, fromP + 999);
          if (!error && data && data.length > 0) {
            userProfiles = userProfiles.concat(data);
      ...
      const pplEligibilityStatusG = { 1: {}, 2: {}, 3: {}, 4: {} };

      const checkPplEligibilityT2 = (uid, g) => {
        const codes = userSlsMap[uid] || [];
        if (codes.length === 0 || capaianT2.length === 0) return false;
        let targetSum = 0;
        let hasilSum = 0;
        let sls100Count = 0;
        codes.forEach(code => {
          targetSum += targetMap[code] || 0;
          hasilSum += hasilMapG[g][code] || 0;
          if ((slsSelesaiMapG[g][code] || 0) >= 100) sls100Count++;
        });
        // Syarat 100% selesai didata
        const eligible = (codes.length > 0 && sls100Count === codes.length) || (targetSum > 0 && hasilSum >= targetSum);
        return eligible && !isOnHold(uid, g);
      };

      const checkPmlEligibilityT2 = (uid, g) => {
        const supervised = Array.from(pmlToPpl[uid] || []);
        if (supervised.length > 0) {
          const allSupervised100 = supervised.every(pid => pplEligibilityStatusG[g][pid]);
          return allSupervised100 && !isOnHold(uid, g);
        }
        // Fallback jika belum dimapping pml_ppl: cek penugasan langsung PML jika ada
        const directCodes = userSlsMap[uid] || [];
        if (directCodes.length > 0) {
          let sls100Count = 0;
          directCodes.forEach(code => {
            if ((slsSelesaiMapG[g][code] || 0) >= 100) sls100Count++;
          });
          return sls100Count === directCodes.length && !isOnHold(uid, g);
        }
        return false;
      };

      [1, 2, 3, 4].forEach(g => {
        const eligibleSet = bappEligibilityMapT2[g];
        // Evaluasi PPL dulu
        userProfiles.filter(u => u.role === 'ppl').forEach(u => {
          const e = checkPplEligibilityT2(u.id, g);
          pplEligibilityStatusG[g][u.id] = e;
          if (g === 1 && e) eligibleSet.add(u.id);
          else if (g === 2 && e && !pplEligibilityStatusG[1][u.id]) eligibleSet.add(u.id);
          else if (g === 3 && e && !pplEligibilityStatusG[1][u.id] && !pplEligibilityStatusG[2][u.id]) eligibleSet.add(u.id);
          else if (g === 4 && e && !pplEligibilityStatusG[1][u.id] && !pplEligibilityStatusG[2][u.id] && !pplEligibilityStatusG[3][u.id]) eligibleSet.add(u.id);
        });

        // Evaluasi PML
        userProfiles.filter(u => u.role === 'pml').forEach(u => {
          const e = checkPmlEligibilityT2(u.id, g);
          if (g === 1 && e) eligibleSet.add(u.id);
          else if (g === 2 && e && !bappEligibilityMapT2[1].has(u.id)) eligibleSet.add(u.id);
          else if (g === 3 && e && !bappEligibilityMapT2[1].has(u.id) && !bappEligibilityMapT2[2].has(u.id)) eligibleSet.add(u.id);
          else if (g === 4 && e && !bappEligibilityMapT2[1].has(u.id) && !bappEligibilityMapT2[2].has(u.id) && !bappEligibilityMapT2[3].has(u.id)) eligibleSet.add(u.id);
        });
      });

      isBappEligibilityLoadedT2 = true;
    } catch (err) {
      console.error('Gagal menghitung kelayakan Termin 2:', err);
    }
  }
  ...
  async function previewBAST(gelombang = 1) {
    generateBASTAction(gelombang, false);
  }

  async function downloadBAST(gelombang = 1) {
    generateBASTAction(gelombang, true);
  }

  async function generateBASTAction(gelombang = 1, isDownload = false) {
    await loadBappEligibilityDataT2(true);
    const eligibleIds = bappEligibilityMapT2[gelombang];
    ...
    showToast(`Memproses BAST Termin II Gelombang ${gelombang}...`, 'info');
    ...
    loadJsPDF(async () => {
      ...
      try {
        ...
        const bastOfficers = await fetchBASTData(gelombang);
        if (bastOfficers.length === 0) {
          showToast('Tidak ada petugas eligible untuk dicetak BAST.', 'warning');
          indicator.remove();
          return;
        }
        ...
        for (let i = 0; i < bastOfficers.length; i++) {
          ...
          indicator.innerHTML = `<span class="spinner" style="width:12px;height:12px;border-width:2px;display:inline-block;"></span> Membuat BAST (${i + 1}/${bastOfficers.length})...`;
          if (i > 0) pdf.addPage("a4", "portrait");
          await buildBASTSingleDocument(pdf, officer, ttdYulianBase64, gelombang);
        }

        indicator.style.borderColor = '#10b981';
        indicator.style.color = '#10b981';
        indicator.innerHTML = '✓ Selesai membuat dokumen BAST!';

        if (isDownload) {
          pdf.save(`bast_termin2_gelombang_${gelombang}.pdf`);
        } else {
          window.open(pdf.output('bloburl'), '_blank');
        }
        setTimeout(() => { indicator.remove(); }, 2000);
  ...
  async function fetchBASTData(gelombang = 1) {
    ...
    const { data: bappT2Uploads } = await db.from('bapp_uploads_t2').select('id, screenshot_uninstall, crop_top_uninstall, crop_bottom_uninstall').in('id', officerIds);
    const bappUploadMap = {};
  ...
  async function buildBASTSingleDocument(pdf, officer, ttdYulianBase64, gelombang = 1) {
    const tanggalDoc = TANGGAL_SURAT_TERMIN2[gelombang] || "11 September 2026";
    let tanggalText = `Lebak, ${tanggalDoc}`;
    const isPml = officer.role === 'pml';
    const roleTitle = isPml ? "Petugas Pemeriksa Lapangan (PML)" : "Petugas Pendataan Lapangan (PPL)";

    pdf.setFont("Bookman", "bold");
    pdf.setFontSize(12);
    pdf.text("BERITA ACARA SERAH TERIMA HASIL PEKERJAAN", 105, 25, { align: "center" });
    pdf.text(`SENSUS EKONOMI 2026 TAHUN 2026`, 105, 31, { align: "center" });

    pdf.setFont("Bookman", "normal");
    pdf.setFontSize(12);
    pdf.text(`Nomor: ${officer.no_bast || "......./BAST/3602/2026"}`, 105, 38, { align: "center" });

    let y = 48;
    const intro = `Pada hari ini, tanggal ${tanggalDoc} bertempat di Badan Pusat Statistik Kabupaten Lebak, yang bertanda tangan di bawah ini:`;
    const introLines = pdf.splitTextToSize(intro, 160);
    drawJustifiedText(pdf, intro, 25, y, 160, 5);
    ...
    pdf.text("1.", 25, y);
    const pihak1 = [
      ["Nama", (officer.nama || "").toUpperCase()],
      ["NIK", officer.nik || "....................................."],
      ["Jabatan", `${roleTitle} Sensus Ekonomi 2026`],
      ["Kecamatan", officer.kecamatan || "....................................."]
    ];
    pihak1.forEach(item => {
      pdf.text(item[0], labelX, y);
      pdf.text(":", colonX, y);
    ...
    pdf.text("selanjutnya disebut sebagai PIHAK PERTAMA.", 30, y);
    y += 7;

    pdf.text("2.", 25, y);
    const pihak2 = [
      ["Nama", "YULIAN SARWO EDI"],
      ["NIP", "197707101999121001"],
      ["Jabatan", "Ketua Tim Pelaksana Sensus Ekonomi 2026 Kabupaten Lebak"]
    ];
    pihak2.forEach(item => {
      pdf.text(item[0], labelX, y);
      pdf.text(":", colonX, y);
    ...
    pdf.text("selanjutnya disebut sebagai PIHAK KEDUA.", 30, y);
    y += 7;

    const pasal = [
      `PIHAK PERTAMA telah menyerahkan seluruh hasil pekerjaan pelaksanaan Sensus Ekonomi 2026 kepada PIHAK KEDUA sesuai dengan Perjanjian Kerja Nomor: ${officer.no_spk || "....................................."}, dengan rincian hasil pekerjaan sebanyak ${officer.realisasi || 0} unit usaha/keluarga selesai dari total target ${officer.target || 0}.`,
      `PIHAK KEDUA telah memeriksa dan menerima hasil pekerjaan tersebut dengan baik, lengkap, dan memenuhi standar mutu yang telah ditetapkan.`,
      `Berita Acara Serah Terima ini dibuat dalam rangkap 2 (dua) untuk dipergunakan sebagaimana mestinya.`
    ];
    ...
    pdf.text("Ketua Tim Pelaksana SE2026", 55, ttdY + 5, { align: "center" });

    pdf.text("PIHAK PERTAMA", 155, ttdY, { align: "center" });
    pdf.text(roleTitle, 155, ttdY + 5, { align: "center" });

    if (ttdYulianBase64) {
      pdf.addImage(ttdYulianBase64, 'PNG', 45, ttdY + 12, 18, 25);
    }

    pdf.text("YULIAN SARWO EDI", 55, ttdY + 40, { align: "center" });
    pdf.text("NIP. 197707101999121001", 55, ttdY + 45, { align: "center" });

    pdf.text((officer.nama || "").toUpperCase(), 155, ttdY + 40, { align: "center" });
    pdf.text(`NIK. ${officer.nik || "....................................."}`, 155, ttdY + 45, { align: "center" });

    // Tambah Halaman Lampiran Foto
    pdf.addPage('a4', 'landscape');
    ...
    pdf.setFont('times', 'bold');
    pdf.setFontSize(12);
    pdf.text('B. Screenshot Aplikasi Fasih KECAMATAN ' + (officer.kecamatan || ".........................").toUpperCase(), 20, 20);
    ...
    const prepareCroppedImage = (src, topOff, botOff) => {
      return new Promise((resolve) => {
        ...
        img.onload = () => {
          ...
          canvas.height = cropHeight;
          ctx.drawImage(img, 0, startY, origW, cropHeight, 0, 0, origW, cropHeight);
          resolve({ base64: canvas.toDataURL('image/png'), ratio: (origH / origW) * (cropHeightPercent / 100) });
        };
        img.onerror = () => resolve(null);
      });
    };
    ...
    // Load Ning signature for BAST lampiran (tengah bawah)
    const loadImgAsBase64 = (url) => {
      return new Promise((resolve) => {
        const img = new Image();
        img.crossOrigin = 'Anonymous';
    ...
    const ttdNingBase64 = await loadImgAsBase64('assets/ttd/ning sl.png') || await loadImgAsBase64('assets/ning_sri_lestari.png');
    if (ttdNingBase64) {
      pdf.addImage(ttdNingBase64, 'PNG', 202, sigY2 + 4, 50, 25);
    }

    pdf.setFont('times', 'bold');
    pdf.text((officer.nama || "").toUpperCase(), 70, sigY2 + 25, { align: 'center' });
    const leftW = pdf.getTextWidth((officer.nama || "").toUpperCase());
    pdf.setLineWidth(0.3);
    pdf.line(70 - leftW / 2, sigY2 + 26, 70 + leftW / 2, sigY2 + 26);

    pdf.text('YULIAN SARWO EDI', 227, sigY2 + 25, { align: 'center' });
    const rightW = pdf.getTextWidth('YULIAN SARWO EDI');
    pdf.line(227 - rightW / 2, sigY2 + 26, 227 + rightW / 2, sigY2 + 26);
  }
